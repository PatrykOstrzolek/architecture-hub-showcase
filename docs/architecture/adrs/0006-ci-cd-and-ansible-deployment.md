...
- `cd-backend.yml` — triggered by `workflow_run` after `ci-backend` passes; builds the Docker image, pushes to GHCR, Trivy-scans the built image itself (distinct from `ci-security.yml`'s filesystem-only scan — see [ADR-0016](0016-ec2-secondary-failover-drill.md)), then runs `ansible/playbooks/deploy.yml` (the `app` role) sequentially against the primary (Mikrus) and, since ADR-0016, the EC2 secondary
- `cd-frontend.yml` — triggered by `workflow_run` after `ci-frontend` passes; deploys to Vercel via `vercel deploy --prod`
- `cd-infra.yml` — **manually triggered** (`workflow_dispatch`) only; runs `ansible/playbooks/provision.yml` (`common`/`docker`/`tailscale`/`nginx` roles — packages, Docker install, Tailscale mesh join, nginx vhosts) against a required `target: primary|secondary` input. Added 2026-07-01 after `app.conf.j2` (nginx) changed and nothing applied it: `deploy.yml` only ever ran the `app` role, and nothing wired `provision.yml` into CI at all, so infra changes silently required someone to SSH in and run Ansible from their own machine by hand — a real gap against this ADR's own "reproducible on any fresh VPS" / "server state is fully described in `ansible/`" goals. Deliberately kept separate from the automatic `cd-backend`/`cd-frontend` (infra changes are rarer and riskier than app deploys and shouldn't fire on every push), but still centralized through GitHub Actions using the same secrets, rather than requiring local SSH/vault access.
...
- No true high availability, even after adding an EC2 secondary ([ADR-0016](0016-ec2-secondary-failover-drill.md)) — both instances share one unreplicated Postgres, and failover is a manual drill, not automatic; acceptable for a showcase project
