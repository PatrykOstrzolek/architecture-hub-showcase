...
- **Status**: Accepted (frontend deployment section superseded by [ADR 0009](0009-vercel-frontend-deployment.md); `cd-infra.yml`/`provision.yml` described below removed by [ADR 0018](0018-drop-server-provisioning-ansible.md))
- **Date**: 2026-06-28
- **Deciders**: Patryk O
...
- `cd-backend.yml` — triggered by `workflow_run` after `ci-backend` passes; builds the Docker image, pushes to GHCR, Trivy-scans the built image itself (distinct from `ci-security.yml`'s filesystem-only scan), then runs `ansible/playbooks/deploy.yml` (the `app` role) against the primary (Mikrus)
...
- No true high availability — a single instance, single unreplicated Postgres, no automatic failover; acceptable for a showcase project
