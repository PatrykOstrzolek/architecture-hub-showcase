...
export function HomeView({ content, articles, page, pages, total }: HomeViewProps) {
  return (
            ...
            {articles.map((item, i) => (
              <li key={item.url ?? i}>
                <Link href={item.url} className="group block h-full">
                  <Card className="h-full transition-shadow group-hover:ring-foreground/20 group-hover:shadow-md">
                    <CardHeader>
          ...
          <nav aria-label="Pagination" className="mt-10 flex items-center justify-center gap-1">
            ...
            {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
              <Button
                ...
                asChild={p !== page}
              >
                {p !== page ? <Link href={`/?page=${p}`}>{p}</Link> : <span>{p}</span>}
              </Button>
            ))}
