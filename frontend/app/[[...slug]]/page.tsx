  ...
  switch (data.template) {
    case "article": {
      ...
      const [lpData, allArticles] = await Promise.all([
        pathSlug ? getContent<LearningPathContent>(`/learning-paths/${pathSlug}`) : Promise.resolve(null),
        getArticles(1, 4),
      ]);
      ...
      if (pathSlug && lpData) {
        ...
        if (index >= 0) {
          learningPath = {
            ...
            prev: index > 0 ? `${articleHref(articles[index - 1])}?path=${encodeURIComponent(pathSlug)}` : null,
            next: index < articles.length - 1 ? `${articleHref(articles[index + 1])}?path=${encodeURIComponent(pathSlug)}` : null,
          };
        }
      }
    ...
    case "authors":
      return <AuthorsListingView content={data.content as unknown as AuthorsListingContent} />;
    case "author":
    ...
    case "learning-paths":
      return <LearningPathsListingView content={data.content as unknown as LearningPathsListingContent} />;
    case "learning-path": {
